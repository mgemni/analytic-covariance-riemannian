...
pipeline_names_lw = {
            "COV(lw)+MDM": "COV(lw)+MDM",
            "ACOV(lwfc)+MDM": "ACOV(lwfc)+MDM",
            "COV(lw)+TSP+LR": "COV(lw)+TS+LR",
            "ACOV(lwfc)+TSH+LR": "ACOV(lwfc)+TSH+LR",
            }
pipeline_names_oas = {
            "COV(oas)+MDM": "COV(oas)+MDM",
            "ACOV(oasc)+MDM": "ACOV(oasc)+MDM",
            "COV(oas)+TSP+LR": "COV(oas)+TS+LR",
            "ACOV(oasc)+TSH+LR": "ACOV(oasc)+TSH+LR",
            }

pipeline_groups = {"no_shrinkage": pipeline_names_no, "lw_shrinkage": pipeline_names_lw, "oas_shrinkage": pipeline_names_oas}


for pipeline_group_name, pipeline_group in pipeline_groups.items():

    print("\n--------------------------------------------------------")
    print("Results for pipeline group:", pipeline_group_name)
    print("--------------------------------------------------------\n")

    pipeline_names = pipeline_group
    ...
    # Save as EPS or SVG
    fig.savefig(results_path + "/EEG_results" + pipeline_group_name + ".eps")
    fig.savefig(results_path + "/EEG_results" + pipeline_group_name + ".svg", bbox_inches='tight', transparent=True)
